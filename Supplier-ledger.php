          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Supllier Ledger Details</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped" data-order='[[ 0, "desc" ]]'>
                  ...
                  <tbody>
                    <?php
                    $query = "SELECT * FROM ledgers WHERE ledger_type='Purchase' AND amount>0 OR amount<0 AND ledger_user='Supplier' ORDER BY id DESC";
                    ...
                    if ($result->num_rows > 0) {
                      while($row = mysqli_fetch_array($result))  
                      {
                        ?>
                        <tr>
                          <td><?php $suppliers=$row['user_no'];
                  $query1 = "SELECT * FROM suppliers WHERE supplier_no='$suppliers'";
                    ...
                    echo $name=$row1['name'];
                        ...
                        <td style="background-color: red;color: white;"><?php 
                          $amt= ltrim($amount, '-');
                          echo 'You need to pay <b style="font-size: 20px">'.$amt.'</b> to '.$name.'.';?></td>
                      <?php }
                      elseif($amount<0){ ?>
                        <td style="background-color: green;color: white;"><?php 
                           echo $name.' needs to pay <b style="font-size: 20px">'.$amount.'</b> to you.';
                        ?></td>
                      <?php
                      }
                      ?></tr><?php
